...
knd = tab1.sidebar.multiselect(
    '> 채권 종류', ('전환사채권', '신주인수권부사채권', '교환사채권')
)
corp_nm = tab1.sidebar.text_input('> 발행사명(전체 기업 검색 시 공란)', '삼성전자')
start_dt = tab1.sidebar.date_input('> 시작일')
end_dt = tab1.sidebar.date_input('> 종료일', min_value=start_dt)
intr_ex = tab1.sidebar.slider('> 표면이자율(%)', 0, 20)
intr_sf = tab1.sidebar.slider('> 만기이자율(%)', 0, 20)

if tab1.sidebar.button('조회'):
